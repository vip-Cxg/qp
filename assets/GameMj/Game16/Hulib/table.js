"use strict";

//const fs = require( 'fs' );

var TABLE = function () {
    this.tbl = {};
};

module.exports = TABLE;

TABLE.prototype.init = function () {
    this.tbl = {};
};

TABLE.prototype.check = function (key) {
    return this.tbl[key] ? true : false;
};

TABLE.prototype.add = function (key) {
    // console.log(key);
    this.tbl[key] = 1;
};

//创建牌型文件
TABLE.prototype.dump = function (name) {

    // var fWrite = fs.createWriteStream(__dirname + '/tbl/' + name);
    // var  c=0;
    // for(let  p  in  this.tbl){
    //     console.log(p);
    //     fWrite.write(p);
    //     fWrite.write('\n');
    //     c++;
    //     // console.log("打印="+this.tbl[p]);
    // }
    // fWrite.end();
    // console.log("Dump File,FileName="+name+";length="+c);
};

//加载牌型
TABLE.prototype.load = function (name) {
    let c = 0;
    //cc.log('loadloadloadloadloadloadload')
    // cc.log('cc.sys.isNative');
    // cc.log(cc.sys.isNative);
    // cc.loader.loadRes(`tbl/${name}`,,(err,res)=>{
    var url = cc.url.raw(`resources/tbl/${name}`); //type: cc.TextAsset,
    cc.loader.loadRes("tbl/"+name, (err, res) => {
    //     console.log("文件名: ",name+"1")
    // cc.loader.loadRes(`tbl/${name}.txt`,(err,res)=>{
        if (err) {
        } else {
            let n = res.text;
            n = n.split('\n');
            //n = n.split( '\r' );
            // if(name == 'table_1'){
            //     cc.log('mnnnnnnnnnnnnnnn');
            //     cc.log(n);
            //     cc.log('mnnnnnnnnnnnnnnn');
            // }
            //cc.log(n.length);
            for (let i = 0; i < n.length; i++) {
                if (n[i]) {
                    n[i] = n[i].replace("\r", "");
                    this.tbl[n[i]] = 1;
                    c++;
                }
            }

        }
    })
    if (!cc.sys.isNative)
        return;
    let n = jsb.fileUtils.getStringFromFile(cc.url.raw(`resources/${name}.json`));
    n = n.split('\n');
    //n = n.split( '\r' );
    // if(name == 'table_1'){
    //     cc.log('mnnnnnnnnnnnnnnn');
    //     cc.log(n);
    //     cc.log('mnnnnnnnnnnnnnnn');
    // }
    //cc.log(n.length);
    for (let i = 0; i < n.length; i++) {
        if (n[i]) {
            n[i] = n[i].replace("\r", "");
            this.tbl[n[i]] = 1;
            c++;
        }
    }
    // if( !fs.existsSync( __dirname + '/tbl/' + name ) )
    // {
    //     console.log( "文件不存在" );
    //     return;
    // }
    //
    // const d = fs.readFileSync( __dirname + '/tbl/' + name );

};